...
private:
    size_t len; // the length of the strings
...
public:
    ...
    // bracket operator overload. returns a char reference instead of DSString
    // without this implemented in test DSString,
    // char c = a[0]; char would be equal to t, so add functionality
     ...
     * @return DSString
     */
    // toLower has an implemented normalize function
    ...
    std::vector<DSString> tokenize(DSString str);
    ...
    friend std::istream &getline(std::istream &in, DSString &c, char delim);
    friend std::istream &getline(std::istream &in, DSString &c);
